...
class Theme_Plugin_Enhancements {
	...
	function __construct() {
		...
		$this->plugins = array(
			array(
				...
				'message' => sprintf(
					__( 'The %1$s is required to use some of this theme&rsquo;s features, including: ', 'sapor' ),
					'<strong>' . __( 'Jetpack plugin', 'sapor' ) . '</strong>' ),
	...
	function get_module_name( $module ) {
		$module_names = array(
			'none'                 => __( 'no specific module needed', 'sapor' ),
			'custom-content-types' => __( 'Custom Content Types module', 'sapor' ),
		);
	...
	function admin_notices() {
		...
		foreach ( $this->plugins as $key => $plugin ) {
			...
			if ( 'to-activate' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					__( ' Please activate %1$s. %2$s', 'sapor' ),
					esc_html( $plugin['name'] ),
					( $activate_url ) ? '<a href="' . $activate_url . '">' . __( 'Activate', 'sapor' ) . '</a>' : ''
				);
			}
			...
			if ( 'to-install' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					__( ' Please install %1$s. %2$s', 'sapor' ),
					esc_html( $plugin['name'] ),
					( $install_url ) ? '<a href="' . $install_url . '">' . __( 'Install', 'sapor' ) . '</a>' : ''
				);
			}
		...
		foreach ( $this->unactivated_modules as $module => $features ) :
			...
			$notice .= '<p>';
			$notice .= sprintf(
				__( 'To use %1$s, please activate the Jetpack plugin&rsquo;s %2$s.', 'sapor' ),
